...
<html>
	...
	<body>
		<script type="text/javascript">
			$Ajax.SWFRequest.write("swf/xhr.swf");	// Ajax 호출을 하기 전에 반드시 플래시 객체를 초기화
		</script>
		<button onclick="getList(); return false;">Get List</button>
 ...
 *
 *
var xhr = $Ajax("http://www.remote.com", {
   onload : function(res) {
      // onload 핸들러
   }
});

xhr.request( {key1:"value1", key2:"value2"} );	// 서버에 전송할 데이터를 매개변수로 넘긴다.
...
var request_type = xhr.option("type");					// type 인 xhr 을 리턴한다.
xhr.option("method", "post");							// method 를 post 로 설정한다.
xhr.option( { timeout : 0, onload : handler_func } );	// timeout 을 으로, onload 를 handler_func 로 설정한다.
 ...
 * @description [Lite]
 * @example
var customheader = xhr.header("myHeader"); 		// HTTP 요청 헤더에서 myHeader 의 값
xhr.header( "myHeader", "someValue" );				// HTTP 요청 헤더의 myHeader 를 someValue 로 설정한다.
xhr.header( { anotherHeader : "someValue2" } );	// HTTP 요청 헤더의 anotherHeader 를 someValue2 로 설정한다.
 ...
 *
 * @param {String} [swf_path] 통신을 담당할 플래시 파일의 경로. 생략하면 기본 값은 "./xhr.swf" 이다.
 *
...
jindo.$Ajax.SWFRequest.write = function(swf_path) {
	if(typeof swf_path == "undefined") swf_path = "./xhr.swf";
